...
export const ScheduleGrid = React.memo(function ScheduleGrid({
  ...
  const handleCellClick = useCallback(
    (shift: string, day: string, slot: number, subject: Subject | null) => {
      if (subject) {
              ...
              <thead className="bg-gray-50 dark:bg-gray-800 sticky top-0 z-10">
                <tr>
                  ...
                  {Object.entries(DAYS_MAP).map(([day, label]) => (
                    <th
                      key={day}
                      className="border border-gray-200 dark:border-gray-700 p-2 bg-gray-100 dark:bg-gray-800 text-gray-700 dark:text-gray-100"
                    >
                      {label}
                    </th>
                  ))}

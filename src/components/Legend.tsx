      ...
      <div className="space-y-4">
        ...
        <div>
          ...
          <table className="w-full text-sm border-separate border-spacing-1">
            <tbody>
              <tr>
                {Object.entries(DAYS_MAP).slice(0, 4).map(([code, name]) => (
                  <td key={code} className="bg-gray-100 dark:bg-gray-700 rounded-lg p-2 text-center align-middle">
                    <div className="font-mono text-xs text-gray-500 dark:text-gray-400">{code}</div>
                    <div className="text-gray-700 dark:text-gray-200 font-medium">{name}</div>
                  </td>
                ))}
              </tr>
              <tr>
                {Object.entries(DAYS_MAP).slice(4).map(([code, name]) => (
                  <td key={code} className="bg-gray-100 dark:bg-gray-700 rounded-lg p-2 text-center align-middle">
                    <div className="font-mono text-xs text-gray-500 dark:text-gray-400">{code}</div>
                    <div className="text-gray-700 dark:text-gray-200 font-medium">{name}</div>
                  </td>
        ...
        <div>
          ...
          {Object.entries(TIME_SLOTS).map(([shift, slots]) => (
            <div key={shift} className={`mb-3 transition-all
            ...
            `}>
              ...
              <table className="w-full text-xs border-separate border-spacing-y-0.5 rounded-lg overflow-hidden">
                <thead>
                  <tr>
                    <th className="bg-gray-200 dark:bg-gray-700 text-gray-700 dark:text-gray-200 font-semibold px-2 py-1 rounded-tl-lg">Nº</th>
                    <th className="bg-gray-200 dark:bg-gray-700 text-gray-700 dark:text-gray-200 font-semibold px-2 py-1 rounded-tr-lg">Horário</th>
                  </tr>
                </thead>
                <tbody>
                  {slots.map((slot, idx) => (
                    <tr key={slot.id} className={idx % 2 === 0 ? 'bg-gray-50 dark:bg-gray-800' : 'bg-white dark:bg-gray-900'}>
                      <td className="font-mono px-2 py-1 text-center align-middle border border-gray-200 dark:border-gray-700 rounded-l-lg">{slot.id}</td>
                      <td className="px-2 py-1 text-gray-600 dark:text-gray-300 align-middle border border-gray-200 dark:border-gray-700 rounded-r-lg">{slot.start} - {slot.end}</td>
                    </tr>
                  ))}
                </tbody>
              </table>
